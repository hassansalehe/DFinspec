...
void Solve()
{
	for (long j = 0; j < NumBlocks; j++)
	{
		...
		parallel_for(	blocked_range<size_t>(j+1,NumBlocks,1),
		...
		parallel_for(	blocked_range<size_t>(0,j,1),
						[&](const blocked_range<size_t>& r) {
			for (unsigned long i = r.begin(); i != r.end(); ++i) {
		...
		for (long k= 0; k< j; k++)
		{
			for (long i = j+1; i < NumBlocks; i++)
			{
				...
				args.push_back(Triplet{i, j, k});
			}
		}

		parallel_for(	blocked_range<size_t>(0, args.size(), args.size()/num_threads),
						[&](const blocked_range<size_t>& r) {
			for (unsigned long _i = r.begin(); _i != r.end(); ++_i) {
				Triplet &t = args[_i];
				sgemmBlock( A[t.i][t.k], A[t.j][t.k], A[t.i][t.j],
						BlockSize);
			}
		});
	}
