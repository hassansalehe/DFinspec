...
struct cellIndex{
  ...
  cellIndex(long r, long c) : row(r), col(c) {}
  cellIndex(const cellIndex &ci) : row(ci.row), col(ci.col) {}
};



struct spotrfNodeBody {
	long operator() (long index) {
		spotrfBlock( A[index][index], BlockSize);
		return index;
	}
};

// diagonal block [j,j]. index goes from 0 to j
struct ssyrkNodeBody {
	typedef tbb::flow::function_node<long, long> type;
	typedef type::output_ports_type output_type;

	long cnt;
	void operator() (long index, output_type out) {
		ssyrkBlock( A[index][index], A[index][index], BlockSize);
		cnt++;
		if (cnt == index-1)
			std::get<0>(out).try_put(index);
	}
};


struct strsmNodeBody {
	long row;
	strsmNodeBody(long r) : row(r) {}

	long operator() (std::tuple<long, cellIndex> input) {
		long col = std::get<0>(input);
		strsmBlock( A[col][col], A[row][col], BlockSize);
		return row;
	}
};

struct sgemmNodeBody {
	cellIndex index;
	sgemmNodeBody(long row, long col) : index(row,col) {}

	cellIndex operator() (std::tuple<long, long> input) {
		long i = index.row;
		long j =index.col;
		long k = std::get<0>(input);
...
void Solve()
{
	...
	/* create graph nodes */
	function_node<long,long> **spotrfNode = new function_node<long,long> * [NumBlocks];
	function_node<long,long> **ssyrkNode = new function_node<long,long> * [NumBlocks];
	function_node<std::tuple<long, cellIndex>, long> ***strsmNode =
			new function_node<std::tuple<long, cellIndex>, long> ** [NumBlocks];
	function_node<std::tuple<long, long>, cellIndex> ***sgemmNode =
			new function_node<std::tuple<long, long>, cellIndex> ** [NumBlocks];

	join_node<std::tuple<long, cellIndex> > ***strsmJoin =
			new join_node<std::tuple<long, cellIndex> > ** [NumBlocks];
	join_node<std::tuple<long, long> > ***sgemmJoin =
			new join_node<std::tuple<long, long> > ** [NumBlocks];



	for (long i = 0; i < NumBlocks; i++) {
		spotrfNode[i] = new function_node<long,long> (g, 1, spotrfNodeBody());
		ssyrkNode[i] = new function_node<long,long> (g, 1, ssyrkNodeBody());

		strsmNode[i] = new function_node<std::tuple<long, cellIndex>, long> * [NumBlocks];
		sgemmNode[i] = new function_node<std::tuple<long, long>, cellIndex> * [NumBlocks];
		strsmJoin[i] = new join_node<std::tuple<long, cellIndex> > * [NumBlocks];
		sgemmJoin[i] = new join_node<std::tuple<long, long> > * [NumBlocks];

		for (long j = 0; j < NumBlocks; j++) {
			strsmNode[i][j] = new function_node<std::tuple<long, cellIndex>, long> (g, 1, strsmNodeBody(i));
			sgemmNode[i][j] = new function_node<std::tuple<long, long>, cellIndex> (g, 1, sgemmNodeBody(i, j));
			strsmJoin[i][j] = new join_node<std::tuple<long, cellIndex> > (g);
			sgemmJoin[i][j] = new join_node<std::tuple<long, long> > (g);
		}
	}

	/* make edges */

	/* connect spotrf input */
	for (long j = 0; j < NumBlocks; j++) {
		make_edge(*ssyrkNode[j], *spotrfNode[j]);
	}

	/* connect ssyrk input */
	for (long j=0; j<NumBlocks; j++) {
		for (long i=0; i< j; i++) {
			make_edge(*strsmNode[j][i], *ssyrkNode[j]);
		}
	}
	...
	for (long j = 0; j < NumBlocks; j++) {
		...
		for (long i = j+1; i < NumBlocks; i++) {
			make_edge(*spotrfNode[j], input_port<0>(*strsmJoin[i][j]));
		}

		for (long i = j+1; i < NumBlocks; i++) {
			make_edge(*sgemmNode[i][j], input_port<1>(*strsmJoin[i][j]));
		}

		for (long i = 0; i < NumBlocks; i++) {
			make_edge(*strsmJoin[i][j], *strsmNode[i][j]);
		}
	}
	...
	for (long j = 0; j < NumBlocks; j++) {
		for (long k= 0; k< j; k++) {
			for (long i = j+1; i < NumBlocks; i++) {
				make_edge(*strsmNode[j][k], input_port<0>(*sgemmJoin[i][j]));
				make_edge(*strsmNode[i][k], input_port<1>(*sgemmJoin[i][j]));
			}
		}
	}
	...
	for (long j = 0; j < NumBlocks; j++) {
		for (long i = 0; i < NumBlocks; i++) {
			make_edge(*sgemmJoin[i][j], *sgemmNode[i][j]);
		}
	}
