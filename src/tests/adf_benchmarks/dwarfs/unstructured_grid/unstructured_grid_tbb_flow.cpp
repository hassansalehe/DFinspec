...
private:
	...
	// tasks
	vector<function_node<int>*>*solverTasks;
...
Solver :: Solver(Configurator* configurator)
{
	...
	solverTasks = new vector<function_node<int>*>();
}
...
Solver :: ~Solver()
{
	...
	for (unsigned int i = 0; i < solverTasks->size(); i++)
		if ((*solverTasks)[i]) delete (*solverTasks)[i];
...
void Solver::SolveTask()
{
	...
	for (unsigned int  k = 0; k < numVertex; k += chunkSize) {
		...
		auto st = new function_node<int> (g, 1, [=] (int solveToken) -> void
		{
			...
			if (do_output)
				for (unsigned int i = 0; i < solverTasks->size(); i++) {
					(*solverTasks)[i]->try_put(solveToken);
				}
		});

		solverTasks->push_back(st);
	}
}
...
void ParseCommandLine(int argc, char **argv)
{
	...
	while ((c = getopt (argc, argv, "hf:n:r")) != -1)
		switch (c) {
			case 'h':
				...
				"\n"
				"Usage:\n"
				"   unstructured_grid_tbb_flow [options ...]\n"
				"\n"
				"Options:\n"
				"   -h\n"
	...
	printf ("\nStarting TBB_FLOW Finite Unstructured Grid.\n");
