...
void Solver::Solve()
{
	...
	for (kk=0; kk<num_blocks; kk++)
	{
		...
		vector<Duplet> fwd_args;
		for (jj=kk+1; jj<num_blocks; jj++)
			if (matrix[kk*num_blocks+jj] != NULL)
			{
				fwd_args.push_back(Duplet{kk*num_blocks+kk, kk*num_blocks+jj});
			}

		/* parallel section */
		parallel_for(blocked_range<size_t>(0,fwd_args.size(),fwd_args.size()/num_threads),
				[&] (const blocked_range<size_t>& r) {
			for (unsigned int i = r.begin(); i != r.end(); i++) {
				Duplet t = fwd_args[i];
				fwd(matrix[t.a], matrix[t.b]);
			}
		});

		vector<Duplet> bdiv_args;
		for (ii=kk+1; ii<num_blocks; ii++)
			if (matrix[ii*num_blocks+kk] != NULL)
			{
				bdiv_args.push_back(Duplet{kk*num_blocks+kk, ii*num_blocks+kk});
			}

		/* parallel section */
		parallel_for(blocked_range<size_t>(0,bdiv_args.size(),bdiv_args.size()/num_threads),
				[&] (const blocked_range<size_t>& r) {
			for (unsigned int i = r.begin(); i != r.end(); i++) {
				Duplet t = bdiv_args[i];
				bdiv(matrix[t.a], matrix[t.b]);
			}
		});

		vector<Triplet> bmod_args;
		for (ii=kk+1; ii<num_blocks; ii++)
			if (matrix[ii*num_blocks+kk] != NULL)
				for (jj=kk+1; jj<num_blocks; jj++)
					if (matrix[kk*num_blocks+jj] != NULL)
					{
						if (matrix[ii*num_blocks+jj]==NULL) matrix[ii*num_blocks+jj] = allocate_clean_block();
						bmod_args.push_back(Triplet{ii*num_blocks+kk, kk*num_blocks+jj, ii*num_blocks+jj});
					}

		/* parallel section */
		parallel_for(blocked_range<size_t>(0,bmod_args.size(),bmod_args.size()/num_threads),
				[&] (const blocked_range<size_t>& r) {
			for (unsigned int i = r.begin(); i != r.end(); i++) {
				Triplet t = bmod_args[i];
				bmod(matrix[t.a], matrix[t.b], matrix[t.c]);
			}
		});
	}
