...
{
	findTask = new function_node<Bounds> (g, 2*num_threads, [=] (Bounds inputToken) -> void
	{
		long myBeginPoint = inputToken.beginPoint;
		long myEndPoint = inputToken.endPoint + stateDimensionCoincidence;
...
void Solver::Solve()
{
	...
	static long  begin = 0;
	long partSize = std::max( contentSize / (num_threads * 100 ), (long) 2 * stateDimensionCoincidence);
...
void ParseCommandLine(int argc, char **argv)
{
	...
	while ((c = getopt (argc, argv, "hf:n:r")) != -1)
		switch (c) {
			case 'h':
				...
				"\n"
				"Usage:\n"
				"   finite_state_machine_tbb_flow [options ...]\n"
				"\n"
				"Options:\n"
				"   -h\n"
	...
	printf ("\nStarting TBB_FLOW Finite State Machine.\n");
