...
void Solver::FindTask()
{
	void *intokens[] = {&findToken};
	adf_create_task(2*num_threads, 1, intokens, [=] (token_t *tokens) -> void
	{
		TRACE_EVENT(14002,142)

		/* TASK START */
		/* recreate context */

		/* redeclare input tokens */
		std::remove_reference<decltype(findToken)>::type findToken;
		memcpy(&findToken, tokens->value, sizeof(findToken));
		...
		long myBeginPoint = findToken.beginPoint;
		long myEndPoint = findToken.endPoint + stateDimensionCoincidence;
		if (myEndPoint > contentSize) myEndPoint = contentSize;

		StatesProcessing(myBeginPoint, myEndPoint);
...
void Solver::InitialTask()
{
	...
	void *outtokens[] = {&findToken};
	adf_create_task(1, 0, NULL, [=](token_t *tokens) -> void
	{
		TRACE_EVENT(14001,141)
		/* TASK START */
		/* recreate context */
		...
		TRANSACTION_BEGIN(2,RW)
			findToken.beginPoint = begin;
			findToken.endPoint = begin + partSize;
			begin += partSize;
		TRANSACTION_END

		/* TASK END */
		adf_pass_token(outtokens[0], &findToken, sizeof(findToken));    /* pass tokens */
...
void ParseCommandLine(int argc, char **argv)
{
	...
	while ((c = getopt (argc, argv, "hf:n:r")) != -1)
		switch (c) {
			case 'h':
				...
				"\n"
				"Usage:\n"
				"   finite_state_machine_adf [options ...]\n"
				"\n"
				"Options:\n"
				"   -h\n"
	...
	printf ("\nStarting ADF Finite State Machine.\n");
