...
void Solver::Solve() {
    ...
    for (;;) {
        parallel_for(blocked_range<size_t>(0, num_threads, 1),
                     [&] (const blocked_range<size_t>& r) {
            for (;;) {
                AssignmentProblem *problem = NULL;
                long localUpperBound = 0;
                ...
                if (problem->getParentLowerBound() >= localUpperBound) {
                    continue;
                }
