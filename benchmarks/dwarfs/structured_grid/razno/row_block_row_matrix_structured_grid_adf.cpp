...
void Solver::SolveTask() {
	int bi, p;
	static int *block_iteration = new int[num_threads]; // per block iteration counter
	static int block_count = 0;

	for (bi = 0; bi < num_threads; bi++) {

			block_iteration[bi] = 0;

			std::vector<int*> addr;
			if (bi > 0 )
				addr.push_back(&(solveToken[bi-1]));
			if (bi < (num_blocks-1))
				addr.push_back(&(solveToken[bi+1]));
			...
			for (uint it = 0; it < addr.size(); it++)
				intokens[it] = addr[it];
			void *outtokens [] = {&generateToken, &(solveToken[bi])};
			...
			adf_create_task(1, p, intokens, [=](token_t *tokens) -> void
			{
				...
				std::remove_reference<decltype(solveToken[bi-1])>::type _solveToken_1;
				std::remove_reference<decltype(solveToken[bi+1])>::type _solveToken_2;
				...
				std::remove_reference<decltype(solveToken[bi])>::type solveToken_self;
				...
				block_iteration[bi] ++;
				if (block_iteration[bi] % num_iterations != 0) {
					...
					solveToken_self = block_iteration[bi];
...
void Solver::GenerateTask() {
	...
	for (bi = 0; bi < num_blocks; bi++) {
			...
			adf_create_task(1, 1, intokens, [=](token_t *tokens) -> void
			{
				...
				std::remove_reference<decltype(solveToken[bi])>::type solveToken;
