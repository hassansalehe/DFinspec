...
void spotrfTask() {
	for (long j=0; j<NumBlocks; j++)
	{
		void *intokens[] = {&ssyrkToken[j]};
		void *outtokens[] = {&spotrfToken[j]};
		adf_create_task(1, 1, intokens, [=] (token_t *tokens) -> void
		{
			...
			std::remove_reference<decltype(ssyrkToken[j])>::type ssyrkToken;
			memcpy(&ssyrkToken, tokens->value, sizeof(ssyrkToken));

			/* redeclare input tokens */
			std::remove_reference<decltype(spotrfToken[j])>::type spotrfToken;
...
//			printf("spotrf [%ld, %ld]\n", j, j);
			...
			spotrf_tile( A[j][j], BlockSize,j);     // Cholesky Factorization of A[j,j]
			spotrfToken = j;
			...
			if (j < NumBlocks - 1)	// do not produce spotrfToken for the last block
				adf_pass_token(outtokens[0], &spotrfToken, sizeof(spotrfToken));    /* pass tokens */
...
void ssyrkTask() {
	for (long j=0; j<NumBlocks; j++)
	{
		void *intokens[] = {&strsmToken[j]};
		void *outtokens[] = {&ssyrkToken[j]};
		adf_create_task(1, 1, intokens, [=] (token_t *tokens) -> void
		{
			TRACE_EVENT(16002,162)
			...
			/* redeclare input tokens */
			std::remove_reference<decltype(strsmToken[j])>::type strsmToken;
			memcpy(&strsmToken, tokens->value, sizeof(strsmToken));
			...
			TRANSACTION_BEGIN(1,RW)
#endif
//			printf("ssyrk [%ld, %ld] using A[%ld , %ld]\n", j, j, j, strsmToken);

			long i = strsmToken;

			ssyrk_tile( A[j][i], A[j][j], BlockSize,j);       // A[j,j] = A[j,j] - A[j,i] * (A[j,i])^t
			...
			/* TASK END */
			if (generate_output)	// do not produce spotrfToken for the last block
				adf_pass_token(outtokens[0], &ssyrkToken, sizeof(ssyrkToken));    /* pass tokens */
...
void strsmTask() {
	for (long j = 0; j < NumBlocks; j++)
	{
		...
		{
			void *intokens[] = {&spotrfToken[j], &sgemmToken[i][j]};
			void *outtokens[] = {&strsmToken[i]};
			adf_create_task(1, 2, intokens, [=] (token_t *tokens) -> void
			{
				...
				/* redeclare input tokens */
				std::remove_reference<decltype(spotrfToken[j])>::type spotrfToken;
				memcpy(&spotrfToken, tokens->value, sizeof(spotrfToken));
				tokens = tokens->next_token;
				std::remove_reference<decltype(sgemmToken[i][j])>::type sgemmToken;
				memcpy(&sgemmToken, tokens->value, sizeof(sgemmToken));

				/* redeclare output tokens */
				std::remove_reference<decltype(strsmToken[i])>::type strsmToken;
				...
				TRANSACTION_BEGIN(1,RW)
#endif
//				printf("strsm [%ld, %ld] using A[%ld , %ld]\n", i, j, j, j);

				strsm_tile( A[j][j], A[i][j], BlockSize,i,j);       // A[i,j] <- A[i,j] = X * (A[j,j])^t
				strsmToken = j;
				...
				/* TASK END */
				adf_pass_token(outtokens[0], &strsmToken, sizeof(strsmToken));    /* pass tokens */
...
void sgemmTask() {
	...
	for (long j = 1; j < NumBlocks; j++)
	{
		for (long i = j+1; i < NumBlocks; i++)
		{
			void *intokens[] = {&strsmToken[j], &strsmToken[i]};
			void *outtokens[] = {&sgemmToken[i][j]};
			adf_create_task(1, 2, intokens, [=] (token_t *tokens) -> void
			{
				TRACE_EVENT(16002,162)
				...
				/* redeclare input tokens */
				std::remove_reference<decltype(strsmToken[i-1])>::type strsmToken_1;
				memcpy(&strsmToken_1, tokens->value, sizeof(strsmToken_1));
				tokens = tokens->next_token;

				std::remove_reference<decltype(strsmToken[i])>::type strsmToken_2;
				memcpy(&strsmToken_2, tokens->value, sizeof(strsmToken_2));
				...
				TRANSACTION_BEGIN(1,RW)
#endif
//				printf("sgemm [%ld , %ld] using A[%ld , %ld] and A[%ld , %ld]\n", i, j, i, strsmToken_1, j, strsmToken_1);
//				if (strsmToken_1 != strsmToken_2)
//					printf("Error in sgemmTask\n");

				long k = strsmToken_1;

				sgemm_tile( A[i][k], A[j][k], A[i][j], BlockSize,i,j);    // A[i,j] = A[i,j] - A[i,k] * (A[j,k])^t
				...
				/* TASK END */
				if (generate_output)
					adf_pass_token(outtokens[0], &sgemmToken, sizeof(sgemmToken));    /* pass tokens */
...
void Initial_sgemmTask() {
	for (long i = 1; i < NumBlocks; i++)
	{
		void *outtokens[] = {&sgemmToken[i][0]};
		adf_create_task(1, 0, NULL, [=] (token_t *tokens) -> void
		{
			...
			/* redeclare output tokens */
			std::remove_reference<decltype(sgemmToken[i][0])>::type sgemmToken;
...
void Initial_ssyrkTask() {

			void *outtokens[] = {&ssyrkToken[0]};
			adf_create_task(1, 0, NULL, [=] (token_t *tokens) -> void
			{
				TRACE_EVENT(16002,162)
				...
				/* redeclare output tokens */
				std::remove_reference<decltype(ssyrkToken[0])>::type ssyrkToken;
...
void Solve() {
	spotrfToken = new long[NumBlocks];
	ssyrkToken = new long[NumBlocks];
	strsmToken = new long[NumBlocks];
	sgemmToken = new long*[NumBlocks];
	for (long i = 0; i < NumBlocks; i++)
		sgemmToken[i] = new long[NumBlocks];
